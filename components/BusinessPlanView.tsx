      ...
      <section className="mb-12">
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="whitespace-pre-wrap text-sm border p-3 bg-slate-50/50 rounded">{summary?.introduction || '내용 없음'}</div>
          </div>
          <div>
            <h3 className="text-lg font-bold mb-2">1.2 사업화 과제 차별성</h3>
            <div className="whitespace-pre-wrap text-sm border p-3 bg-slate-50/50 rounded">{summary?.differentiation || '내용 없음'}</div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <h3 className="text-lg font-bold mb-2">1.3 목표 시장</h3>
              <div className="whitespace-pre-wrap text-sm border p-3 bg-slate-50/50 rounded h-full">{summary?.targetMarket || '내용 없음'}</div>
            </div>
            <div>
              <h3 className="text-lg font-bold mb-2">1.4 달성 목표</h3>
              <div className="whitespace-pre-wrap text-sm border p-3 bg-slate-50/50 rounded h-full">{summary?.goals || '내용 없음'}</div>
            </div>
          </div>
          <div className="mt-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              {images && images.length > 0 ? images.slice(0, 4).map((img, idx) => (
                ...
                <div className="col-span-2 border-2 border-dashed border-slate-200 rounded-lg h-40 flex items-center justify-center text-slate-400 italic">
                  이미지를 생성하지 못했습니다. (API 키 및 네트워크 확인 필요)
                </div>
              )}
            </div>
          </div>
      ...
      <section className="mb-12 page-break">
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="whitespace-pre-wrap text-sm leading-6 text-justify border p-4 bg-slate-50/30 rounded">{problem?.motivation || '데이터 생성 중 오류가 발생했습니다.'}</div>
          </div>
          <div className="pt-4">
            <h3 className="text-lg font-bold mb-2">2.2 제품 및 서비스의 목적 및 필요성</h3>
            <div className="whitespace-pre-wrap text-sm leading-6 text-justify border p-4 bg-slate-50/30 rounded">{problem?.purpose || '데이터 생성 중 오류가 발생했습니다.'}</div>
          </div>
        </div>
      </section>
      ...
      <section className="mb-12 page-break">
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="whitespace-pre-wrap text-sm mb-4 leading-6">{solution?.devPlan || '내용 없음'}</div>
            <h4 className="font-semibold mb-2">3.1.1 단계별 개발 및 협력체 구성</h4>
            <div className="whitespace-pre-wrap text-sm mb-4 leading-6">{solution?.stepwisePlan || '내용 없음'}</div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-slate-300 text-xs">
                ...
                <tbody>
                  {(solution?.budgetTable || []).map((row, i) => (
                    <tr key={i}>
                      <td className="border border-slate-300 p-2 font-bold">{row.item}</td>
          ...
          <div className="pt-4">
            ...
            <div className="whitespace-pre-wrap text-sm mb-4 leading-6">{solution?.customerResponse || '내용 없음'}</div>
            <h4 className="font-semibold mb-2">경쟁사 및 시장 환경 분석</h4>
            <div className="whitespace-pre-wrap text-sm leading-6">{solution?.competitorAnalysis || '내용 없음'}</div>
          </div>
        </div>
      </section>
      ...
      <section className="mb-12 page-break">
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="whitespace-pre-wrap text-sm mb-4 leading-6">{scaleUp?.fundingPlan || '내용 없음'}</div>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-slate-300 text-xs">
                <thead>
                  <tr className="bg-slate-100">
                ...
                <tbody>
                  {(scaleUp?.detailedBudget || []).map((row, i) => (
                    <tr key={i}>
                      <td className="border border-slate-300 p-2 font-bold">{row.category}</td>
                  ...
                  <tr className="bg-blue-50 font-extrabold">
                    <td colSpan={2} className="border border-slate-300 p-2 text-center">합계</td>
                    <td className="border border-slate-300 p-2 text-right">
                      {(scaleUp?.detailedBudget || []).reduce((sum, b) => sum + (b.amount || 0), 0).toLocaleString()}
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={scaleUp?.marketResearchDomestic || []}>
                    ...
                    <Tooltip />
                    <Bar dataKey="value">
                      {(scaleUp?.marketResearchDomestic || []).map((entry, index) => (
                        <Cell key={`cell-dom-${index}`} fill={COLORS[index % COLORS.length]} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                <p className="text-[10px] text-center mt-1 font-bold">내수 시장 규모 전망 (AI 추정)</p>
              </div>
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={scaleUp?.marketResearchGlobal || []}>
                    ...
                    <Tooltip />
                    <Bar dataKey="value">
                      {(scaleUp?.marketResearchGlobal || []).map((entry, index) => (
                        <Cell key={`cell-glob-${index}`} fill={COLORS[(index + 2) % COLORS.length]} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                <p className="text-[10px] text-center mt-1 font-bold">글로벌 시장 성장 지표 (AI 추정)</p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mt-8">
              <div className="bg-slate-50 p-4 rounded-lg border">
                <h4 className="font-bold mb-2">내수 진출 전략</h4>
                <div className="whitespace-pre-wrap text-xs leading-5">{scaleUp?.marketApproachDomestic || '내용 없음'}</div>
              </div>
              <div className="bg-slate-50 p-4 rounded-lg border">
                <h4 className="font-bold mb-2">글로벌 진출 전략</h4>
                <div className="whitespace-pre-wrap text-xs leading-5">{scaleUp?.marketApproachGlobal || '내용 없음'}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="page-break">
        ...
        <div className="space-y-6">
          <div className="pt-2">
            ...
            <div className="whitespace-pre-wrap text-sm leading-6 border p-4 rounded bg-slate-50/20">{team?.capability || '내용 없음'}</div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
            <div>
              <h3 className="text-lg font-bold mb-2">5.2 현 재직 및 고용 계획</h3>
              <div className="whitespace-pre-wrap text-sm leading-6 border p-4 rounded bg-slate-50/20">{team?.hiringStatus || '내용 없음'}</div>
            </div>
            <div>
              <h3 className="text-lg font-bold mb-2">5.3 사회적 가치 실현 (ESG)</h3>
              <div className="whitespace-pre-wrap text-sm leading-6 border p-4 rounded bg-slate-50/20">{team?.socialValue || '내용 없음'}</div>
            </div>
          </div>
        </div>
      </section>

      <div className="mt-16 pt-8 border-t border-slate-200 text-center text-slate-400 text-[10px]">
        본 사업계획서는 SS창업경영연구소 & (주)소셜위즈의 AI 기술을 통해 생성되었습니다. 실제 제출 시 전문가의 검토를 권장합니다.
      </div>
    </div>
  );
};
